...
package is currently `panelary`; the public rename to `panelary` is planned but not yet
effective.
...
- **`pk.detect` raised `AttributeError`.** `detect` — 0.4.0's headline feature,
  37 public symbols — was never added to `panelary/__init__.py`. Nine
...
- **`import panelary.llm` required an API key.** The OpenAI client was
  ...
  `panelary[llm]`, was not a pasteable distribution name).
...
- **`pip install panelary` now installs only `numpy` + `polars`.**
  ...
  `progress`). Feature code imports them lazily via `panelary._deps.require`,
  which raises a single actionable `pip install 'panelary[<extra>]'` message
  when one is missing. **To restore the previous batteries-included behavior:**
  `pip install 'panelary[recommended]'` (ml + scipy + seasonality + cafe) or
  `[all]`.
...
- **The compiled Rust extension (`src/`, Cargo, maturin) has been removed.** Panelary
  ...
  *slower* than numpy, and Lempel–Ziv showed no gain. The only real win was CUSUM, now
  ...
  extra (`pip install 'panelary[fast]'`). Build backend switched maturin → hatchling.

### Changed — packaging & lighter import

- **Lighter `import panelary`.** `scipy`, `scikit-learn`, and `cloudpickle`
  are no longer imported at `import panelary` time; heavy back-ends are now
  loaded lazily at first use via `panelary._deps.require(...)`, which raises a
  single actionable `pip install 'panelary[<extra>]'` hint when a dependency
  is missing.
...
- The booster / ANN extras (`lightgbm`, `catboost`, `xgboost`, `ann`) previously did
  ...
  `pip install 'panelary[lightgbm]'` (etc.) actually work.
...
### Added — clustering (`panelary.cluster`)
...
### Added — dimensionality reduction (`panelary.reduce`)
...
### Added — unsupervised feature selection (`panelary.select`)
...
### Added — cross-sectional / factor evaluation (`panelary.factor`)
...
- **Fractional differencing**: consolidated to a single `ffd_weights` / `frac_diff_expr`
  source of truth (`panelary._ffd`); all five Python surfaces now agree bit-for-bit
...
First release of the Panelary panel core, plus the leak-safe feature-engineering,
...
- CAFE imputation: `cafe_impute` (`panelary.preprocessing`) and `CafeImputer`
  (`panelary.imputation`), powered by the optional `cafe-impute` dependency
  (MIT, Sov.ai) via the new `cafe` extra.
- Bulk `extract_features` (`panelary.feature_extractors`).
- Cross-sectional `.xs` operations.
- Triple-barrier labeling: `triple_barrier` (`panelary.label`).
- catch22 feature set (`panelary.catch22`) — clean-room from Lubba et al. 2019.
- Leakage verifier `assert_no_lookahead` (`panelary.testing`).
- CPCV runner: `cross_validate` / `validate` / `CVReport`
  (`panelary.core.model_selection`).

### Changed
- Bumped version to `0.2.0` (past the live PyPI `0.1.7`); aligned `Cargo.toml`.
- Fixed `__version__` (was functime's `0.9.5`) and the package docstring to describe Panelary.
...
- Packaging hygiene: removed the committed prebuilt wheel from git and ignore `*.whl`;
  fixed the stale `Makefile` (uninstall `panelary`, `BASE ?= main`, `[dev]` extra);
  fixed user-facing `pip install functime[...]` strings to `panelary[...]`.
- Documentation: mkdocstrings API pages now target `panelary.*`; `mkdocs.yml`
  site/repo URLs point at the Panelary / sovai-research locations.
...
- `llms.txt` discovery file (per the llms.txt convention) summarizing Panelary, its
  module taxonomy, and core concepts for coding agents.
- `CHANGELOG.md` (this file).
- Panelary-branded documentation: `docs/quickstart.md` and `docs/leakage.md`
  (correctness-by-construction narrative).

#### Changed
- Repositioned the project as **Panelary** — "leak-safe, Rust-fast feature engineering
  and ML for panel data, built on Polars" — across `README.md`, `mkdocs.yml`, and docs.
- Rewrote `README.md` with the Panelary one-liner/tagline, prominent functime credit,
  install guidance (note: import name remains `panelary`), a 60-second quickstart
  of the target panel API, an honest shipped-vs-roadmap table, and ecosystem positioning.
- Updated `mkdocs.yml` site name/description and navigation for Panelary.
...
#### Notes
- No public Python/Rust APIs were renamed in Phase 0. The `panelary` rename is deferred.
...
The first Panelary release: a drop-in, **maintained** functime with modern packaging.
...
- Documentation site rebranded to Panelary while retaining the functime API surface.
...
- Public `panelary` package rename.

[Unreleased]: https://github.com/sovai-research/panelary/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/sovai-research/panelary/releases/tag/v0.2.0

"""General-purpose time-series feature functions.

The tsfresh-style catalogue: ~70 pure functions of a single series, each usable
as a bare :class:`polars.Expr` inside ``.over(entity_col)``. They are surfaced
on the ``.ts`` expression namespace by :mod:`._namespace`.
"""

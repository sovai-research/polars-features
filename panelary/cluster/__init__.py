"""Leak-safe panel clustering for Panelary.

Two clustering modes, both honouring the
:class:`~panelary.core.protocol.PanelEstimator` contract (fit-on-train,
...
def _warn_unavailable(component: str, exc: Exception) -> None:
    warnings.warn(
        f"panelary.cluster: optional component {component!r} is "
...
try:
    from panelary.cluster._kshape import ncc, sbd
    from panelary.cluster.kshape import KShapeClusterer, k_from_n_entities
...
try:
    from panelary.cluster.cross_sectional import CrossSectionalClusterer
except ImportError as exc:  # pragma: no cover - defensive

...
2. **Cross-fitting.** The nuisance predictions used for observation ``i`` are
   ...
   from Panelary's **purged and embargoed** splitters
   (:class:`~panelary.core.model_selection.PurgedKFold`), so the folds are
...
from panelary.econ._common import factorize, norm_cdf, norm_ppf, ols
...
def dml_partial_linear(
    ...
    *,
    ...
    :class:`~panelary.core.model_selection.PurgedKFold` over the sorted
    ...
    # Fold source: Panelary's purged/embargoed splitter over the unique times.

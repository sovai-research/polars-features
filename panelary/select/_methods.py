...
* :func:`mda` -- Mean-Decrease-Accuracy importance (permutation importance)
  ...
  :class:`~panelary.core.model_selection.PurgedKFold`), so every
...
:class:`~panelary.core.protocol.PanelTransformer` so feature selection can
be dropped into a :class:`~panelary.core.pipeline.Pipeline` as a
``"select"`` step.
...
def mrmr(
    ...
    *,
    ...
    X : PanelFrame | polars.DataFrame | polars.LazyFrame
        ...
        frames are wrapped with :func:`~panelary.core.panel_frame.as_panel`
...
def mda(
    ...
    *,
    ...
    cv : object
        ...
        :class:`~panelary.core.model_selection.PurgedKFold`.
    features : sequence of str, optional

...
(:mod:`._compile`); it manipulates :class:`~panelary.evolve._types.Gene`
tuples and integers only.

Why a fixed-length linear program over a shared DAG
---------------------------------------------------
The representation is fixed in :class:`~panelary.evolve._types.Genome`:
...
---------------------
* ``max_depth`` (default :data:`~panelary.evolve._types.MAX_DEPTH_DEFAULT`
...
:data:`~panelary.evolve._types.Unit`. A gene may only be created if the
...
:data:`~panelary.evolve._types.DIMENSIONLESS` are freely

...
--------------------------------------------------
A "case" in :class:`~panelary.evolve._types.CaseMatrix` is a
...
def _errors_and_epsilon(
    ...
    The lexicase literature is written for *error* (lower is better); Panelary
...
def eps_lexicase(
    ...
    *,
    ...
    scores : ndarray of shape (n_pop, n_cases), or CaseMatrix
        **Higher is better** -- Panelary's convention, e.g. per-cell rank IC.
...
def fast_non_dominated_sort(objectives: np.ndarray) -> list[np.ndarray]:
    ...
    **Convention: every column is maximised.** ``i`` dominates ``j`` iff ``i``
    ...
    the input with sign flips already applied -- the canonical Panelary report
...
def _cvt_centroids(
    ...
    Panelary's ``{numpy, polars}`` core (see ``AGENTS.md``). Cached because the
...
class Archive:
    ...
    ``binning="cvt"`` (default)
        ...
        (Vassiliades et al. 2018). **Essential here**: Panelary uses 5

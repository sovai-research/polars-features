...
:class:`~panelary.core.protocol.PanelTransformer` contract is that the
*reference set* becomes fold-bound automatically: ``fit(train)`` builds the
:class:`~panelary.explain.TimeAwareBackground` from the training rows and
freezes it; ``transform(test)`` computes attributions against that frozen,
past-only reference and learns nothing. Drop it into a
:class:`~panelary.core.pipeline.Pipeline` behind a purged splitter and
...
from panelary.explain._common import SHAP_PREFIX, resolve_features
from panelary.explain._tree import check_efficiency, tree_attributions
...
class TreeAttributor(PanelTransformer):
    ...
    model : object
        ...
        :class:`~panelary.models.PanelLGBMRegressor`), a bare fitted
        booster / sklearn tree ensemble, or any object exposing
        ``panelary_shap_values(X, background)``. The attributor never trains
        anything.
    ...
    mode : {"interventional", "conditional", "path_dependent"}, default="interventional"
        ...
        :class:`~panelary.explain.TimeAwareBackground`.
    ...
    def _fit(self, panel: PanelFrame) -> None:
        from panelary.explain._common import unwrap_model

...
reducer in :mod:`polars_features.reduce`. It mirrors
:class:`polars_features.models._PanelSupervised`: a thin, panel-aware wrapper
...
the :class:`~polars_features.core.protocol.PanelTransformer` contract.
...
* imputation is *not* done here by look-ahead ``bfill`` -- callers compose a
  leak-safe :class:`~polars_features.imputation.CafeImputer` upstream instead.

...
All four share :class:`~polars_features.reduce._estimators._FactorExtractor`, which owns the entire leak-safety
...
which is what separates this family from :mod:`polars_features.select`, whose
...
from polars_features.reduce._hfa import hfa_cumulant_matrix
...
def pca_factors(
    ...
    *,
    ...
    r : int, optional
        ...
        criterion (:func:`polars_features.reduce.n_factors`).
...
class _FactorExtractor(PanelTransformer):
    ...
    keep : {"all", "factors"}, default "all"
        ...
        extractor drops into a :class:`~polars_features.core.pipeline.Pipeline`
...
class PCAFactors(_FactorExtractor):
    ...
    :class:`~polars_features.reduce._estimators._FactorExtractor`.

    Examples
    --------
    >>> import polars as pl
    >>> from polars_features.reduce import PCAFactors
    >>> df = pl.DataFrame(
    ...     {
    ...         "id": ["a"] * 6,
    ...         "t": range(6),
...
class HFAFactors(_FactorExtractor):
    ...
    :mod:`polars_features.reduce._hfa` for the estimator equations, the memory
    ...
    order : {3, 4}, default 3
        ...
        **heavy-tailed** ones. The name matches ``max_order`` in the attribution
        half of PanelKit's interactions theme.
...
class ICAFactors(_FactorExtractor):
    ...
    'polars-features[ml]'``). ``n_factors=None`` uses Bai--Ng ``IC_p2``.
...
class RobustPCAFactors(_FactorExtractor):
    ...
    Pure NumPy IRLS; see :mod:`polars_features.reduce._robust` for the

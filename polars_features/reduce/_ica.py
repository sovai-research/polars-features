...
:class:`~polars_features.core.protocol.PanelTransformer` wrapper and the same
pipelines.

``scikit-learn``'s ``FastICA`` does the heavy lifting and is **imported lazily**
through :func:`polars_features._deps.require`, so ``import polars_features``
stays numpy+polars only. Install it with ``pip install 'polars-features[ml]'``.
"""
...
from polars_features.reduce._common import fix_signs, prepare_matrix

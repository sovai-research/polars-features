...
This module is the *positional* face of PanelKit's cross-validation machinery.
:mod:`polars_features.core.model_selection` already implements purging, embargo,
:class:`~polars_features.core.model_selection.PurgedKFold` and
:class:`~polars_features.core.model_selection.CombinatorialPurgedCV` over
:class:`~polars_features.core.panel_frame.PanelFrame` objects; those are
...
* :func:`cpcv_splits` / :func:`walk_forward_splits` — purged + embargoed splits
  ...
  (:mod:`polars_features.cross_validation`) do **not** purge; these do.
...
* :func:`cpcv_backtest_paths` / :func:`walk_forward_backtest_path` — turn a
  ...
  direct input to
  :func:`~polars_features.validation.probability_of_backtest_overfitting`.
...
def cpcv_splits(
    ...
    *,
    ...
    :class:`~polars_features.core.model_selection.CombinatorialPurgedCV`: the
...
def fold_boundaries(splits: Sequence[IndexSplit]) -> list[int]:
    ...
    :mod:`polars_features.validation._bootstrap`.

...
output of :func:`polars_features.detect._bsadf.bsadf_panel` -- into panel
...
from polars_features.econ._common import pinv_sym, winsorize
...
def breadth(
    ...
    *,
    ...
    stat : ndarray, shape (N, T) or (T,)
        ...
        :func:`polars_features.detect._bsadf.bsadf_panel`. Non-finite entries
    ...
    **Choosing ``cv`` sets the scale of the reading.** A per-endpoint
    ...
    :func:`polars_features.detect._critvals.training_max_cv` or a sup-quantile of
...
def _sign_of_max_abs(vec: NDArray[Any]) -> float:
    ...
    Mirrors :func:`polars_features.reduce._base._sign_of_max_abs` so that
    component orientation is fixed by the same deterministic convention used by
    every other reducer in PanelKit: components stay stable across refits and
...
def residualise(
    ...
    **Sign convention.** Components are oriented by
    :func:`polars_features.reduce._base._sign_of_max_abs`: the largest-magnitude
    loading of each component is forced positive. This is the same convention
    every reducer in :mod:`polars_features.reduce` uses, so component identity
...
def sieve_bootstrap_cv(
    ...
    **bsadf_kwargs: Any,
    ...
    min_window : int
        ...
        :func:`polars_features.detect._bsadf.bsadf_panel`.
    lag : int
    ...
    entity_cv : ndarray, optional
        ...
        :func:`polars_features.detect._critvals.mc_table` at ``entity_level``.
    ...
    * **Correct panel critical values for the mean statistic are near zero or
    ...
    """
    from polars_features.detect._bsadf import bsadf_panel
    ...
    if statistic == "breadth":
        if entity_cv is None:
            from polars_features.detect._critvals import mc_table

"""Shared, dependency-free numerical helpers for :mod:`polars_features.econ`.

Everything here is **pure NumPy**. No
``scipy`` / ``statsmodels`` / ``linearmodels``: the econometric estimators in
this package must work with PanelKit's ``{numpy, polars}`` core.
...
from polars_features._numpy_stats import norm_cdf, norm_ppf

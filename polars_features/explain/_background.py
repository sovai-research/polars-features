...
**past-only, per entity's own calendar**
    ...
    :class:`~polars_features.core.model_selection.PurgedKFold` (embargo counts
...
``conditional``
    ...
    PanelKit binds and audits**: ``conditional`` still requires a fold-bound,
    past-only background, which is used for the reported expected value and for
    :meth:`~polars_features.explain.TreeAttributor.check_efficiency`.
``path_dependent``
...
from polars_features.explain._common import TimeAxis, resolve_features
...
class TimeAwareBackground:
    ...
    embargo : int, default=0
        ...
        before sampling, mirroring the purge/embargo used by PanelKit's purged
        cross-validators.
    seed : int, default=0
    ...
    --------
    >>> import polars as pl
    >>> from polars_features.explain import TimeAwareBackground
    >>> train = pl.DataFrame(
        ...
        *,
        ...
        if mode == "path_dependent" and not i_accept_path_dependent_background:
            raise ValueError(
                ...
                "future of the observation being explained. PanelKit will not "
                "select it silently.\n\n"
        ...
        if mode == "path_dependent":
            warnings.warn(
                ...
                "reference, which PanelKit cannot audit for look-ahead.",
                UserWarning,
                stacklevel=2,
            )

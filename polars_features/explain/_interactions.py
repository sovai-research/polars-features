...
subpackage exists to avoid. What PanelKit adds is the same thing it adds to
...
--------------------------------------
PanelKit uses one word, *interactions*, for higher-order structure on both sides
...
* **data side (:mod:`polars_features.reduce`):** ``order=3|4`` -- the cumulant
...
----------------------
``shapiq``'s imputer knob is mirrored onto PanelKit's
:class:`~polars_features.explain.TimeAwareBackground` mode:

=====================  ===========================================================
PanelKit ``mode``      ``shapiq`` engine
=====================  ===========================================================
...
def _iv_items(iv: Any) -> list[tuple[tuple[int, ...], float]]:
    ...
    if lookup is None or values is None:  # pragma: no cover - shapiq API drift
        raise TypeError(
            ...
            f"({type(iv).__name__}); this PanelKit build expects `dict_values` "
...
def interaction_values(
    ...
    **explainer_kwargs: Any,
    ...
    ----------
    model : object
        A fitted PanelKit model wrapper or bare estimator.
    X : panel
    ...
    max_order : int, default=2
        Maximum interaction order (``2`` = pairwise). PanelKit uses
        ``max_order`` on the model side and ``order`` on the data side
        (:mod:`polars_features.reduce`) -- see the module docstring.
    ...
    **explainer_kwargs
    ...
    if background.mode == "path_dependent":
        ...
        from polars_features.explain._tree import _group_rows_by_background

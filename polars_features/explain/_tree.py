"""Native, exact TreeSHAP dispatch -- PanelKit reimplements none of the math.

Exact TreeSHAP already ships **inside** the boosters PanelKit wraps, and the
...
custom                  any object exposing ``panelkit_shap_values(X, background)``
======================  ==========================================================

Every one of those imports is lazy (:func:`polars_features._deps.require`), so
``import polars_features.explain`` costs nothing but numpy + polars.

What PanelKit *adds* is the argument the libraries leave to the user: the
reference set. See :class:`~polars_features.explain.TimeAwareBackground`.
"""
...
from polars_features.explain._common import (
    BASE_VALUE_COL,
    CUSTOM_HOOK,
...
def tree_attributions(
    ...
    *,
    ...
    """Exact, native TreeSHAP for a fitted PanelKit model, keyed by ``(entity, time)``.

    Parameters
    ----------
    model : object
        A **fitted** PanelKit model wrapper (e.g.
        :class:`~polars_features.models.PanelLGBMRegressor`) or a bare fitted
        booster / sklearn tree ensemble. PanelKit wrappers carry their resolved
    ...
    if background.mode in ("path_dependent", "conditional"):
        # Same exact tree kernel; the difference is what PanelKit binds and

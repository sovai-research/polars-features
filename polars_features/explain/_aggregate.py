...
**Group SHAP with an honest ``group_mode``.**
    ...
    past-only :class:`~polars_features.explain.TimeAwareBackground` as the
    marginal imputer. Conflating the two is a real and common error, so PanelKit
    makes you choose.
...
def group_shap(
    ...
    *,
    ...
    shap : polars.DataFrame
        ...
        :func:`~polars_features.explain.tree_attributions` (keys +

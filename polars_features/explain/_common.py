"""Shared helpers for :mod:`polars_features.explain`.
...
:func:`polars_features._deps.require` inside the function that needs it.
"""
...
class TimeAxis:
    ...
    Used by :class:`~polars_features.explain._background.TimeAwareBackground` to
    ...
    def cutoff(self, t: Any, *, embargo: int = 0) -> int:
        ...
        purge/embargo convention PanelKit's purged CV uses, expressed in
...
#:
#:     panelkit_shap_values(X: np.ndarray, background: np.ndarray | None)
#:         -> tuple[np.ndarray, np.ndarray]   # (phi[n, f], base[n])
CUSTOM_HOOK = "panelkit_shap_values"


def unwrap_model(model: Any) -> tuple[Any, list[str] | None]:
    """Unwrap a PanelKit model wrapper into ``(native_estimator, features)``.

    Accepts either a fitted :mod:`polars_features.models` wrapper (anything with
    ...
    """
    if model is None:
        raise ValueError("`model` is required: pass a fitted PanelKit estimator.")
...
def model_family(estimator: Any) -> str:
    ...
    ``import polars_features.explain`` never drags a booster into ``sys.modules``.
    """
